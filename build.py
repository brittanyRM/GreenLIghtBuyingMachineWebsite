...
CSS = r"""
  ...
  *{box-sizing:border-box}
  ...
  .lp-stack{max-width:66rem;margin-inline:auto}
  .lp-stack .kicker{text-align:center;display:block}

  .lp-below{display:grid;gap:clamp(1.75rem,4vw,3rem);grid-template-columns:1fr;
    margin-top:clamp(2rem,4vw,2.75rem);align-items:start}
  @media(min-width:58rem){
    .lp-below{grid-template-columns:minmax(0,24rem) minmax(0,1fr)}
    /* form sits left on desktop; headline reads first on mobile */
    .lp-below .card{order:-1}
  }
  .lp-words h1{margin:0 0 1rem}
  .lp-words .lede{margin-bottom:1rem}
  .lp-words .lede:last-child{margin-bottom:0}

  .video-wrap{max-width:30rem;margin-inline:auto}
  .vid{width:100%;height:auto;display:block;background:#000;border-radius:3px;
    border:1px solid rgba(232,231,224,.14);
    filter:brightness(.93) saturate(.97) contrast(1.02)}
  .vid-cap{font-family:var(--plan);font-size:.75rem;opacity:.5;margin:.75rem 0 0;
